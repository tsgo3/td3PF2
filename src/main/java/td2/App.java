...
public class App {
    ...
    public static final Function<Annee, Stream<Matiere>> matieresA = mat -> mat.ues().stream().flatMap(e -> e.ects().keySet().stream());
    // matières d'un étudiant
    public static final Function<Etudiant, Stream<Matiere>> matieresE = mat -> mat.notes().keySet().stream();
    // matières coefficientées d'un étudiant (version Entry)
    public static final Function<Etudiant, Stream<Entry<Matiere, Integer>>>
    matieresCoefE_ = mat -> mat.annee().ues().stream().flatMap(e -> e.ects().stream());
